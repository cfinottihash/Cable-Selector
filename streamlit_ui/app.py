...
from tools.iso_estimator import by_bitola          # accepts reinforced=…
import tools.iso_estimator as iso                  # for iso.tol(..)
...
reinforced = False
if voltage == "35 kV":
    reinforced = st.checkbox("Isolação reforçada — cabos eólicos")
...
if mode == "Ø sobre isolação":
    ...
    s_mm2 = None
    st.info(f"Você digitou Ø sobre isolação: **{d_iso} mm ± {iso.tol(voltage)} mm**")
else:
